...
As a follow-up, how would you write an algorithm to
    - detect repeated pairs in 3 sessions
    - detect repeated pairs in any number of sessions 
    - write an O(N) time algorithm to determine how often the matcher created each pair 
...
assume matching inner arr's may not have the str's in the same order
...
create map from session1
    ...
    key = 1st of the pair alphabetically -> if using built-in function, this would increase time complexity
...
** if not done w/ alphabetical check, problem can be done in O(n) time & space **
    -> O(n) time due to looping arr's, where n = length of arr
    -> O(n) space due to creation of new map, where n = map size

*/

function hasRepeatTangoPartner(firstSession, secondSession) { 
    // create new map to hold str's of firstSession as key/value pairs
    let session1 = new Map()

    // iterate firstSession
    for (let i = 0; i < firstSession.length; i++) {
        // initiate var's to hold current inner arr, 1st element of inner arr, & 2nd element of inner arr
    ...
    for (let i = 0; i < secondSession.length; i++) {
        ...
        if (session1.has(firstElement) && session1.get(firstElement) === secondElement || 
            session1.has(secondElement) && session1.get(secondElement) === firstElement) {
                return true
        }
    }

    // return false when all map values are checked against arr str's
    return false
}
